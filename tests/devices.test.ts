...
let server: any;
  ...
  afterAll((done) => {
    server.close(done); // Close the server when tests are done
  });

  describe("setupDevices", () => {
    it("should call the necessary functions", async () => {
      const fetchSemaphoresSpy = jest.spyOn(require('../src/simulator/devices'), 'fetchSemaphores');
      const showSemaphoresSpy = jest.spyOn(require('../src/simulator/devices'), 'showSemaphores');
      const initializeSemaphoresSpy = jest.spyOn(require('../src/simulator/devices'), 'initializeSemaphores');
      const dashboardSpy = jest.spyOn(require('../src/simulator/devices'), 'dashboard');
      const toggleSemaphoresSpy = jest.spyOn(require('../src/simulator/devices'), 'toggleSemaphores');
  ...
  describe("fetchSemaphores", () => {
    it("should fetch semaphores and add them to the list", async () => {
      ...
      (fetchSemaphores as jest.Mock).mockImplementation(() => {
        semaphores.push(
          { uuid: "semaphore1", colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, carCount: 0, emergency: false, description: "", fator: 1 },
          { uuid: "semaphore2", colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, carCount: 0, emergency: false, description: "", fator: 1 }
        );
      });
      
      await fetchSemaphores();
      
      expect(fetchSpy).toHaveBeenCalledWith("http://10.10.10.104:8000/discovery/resources?capability=semaphore");
  ...
  describe("showSemaphores", () => {
    it("should log the semaphores with their factors", () => {
      ...
      semaphores.push(
        { uuid: "semaphore1", description: "Semaphore-1", carCount: 10, colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, emergency: false, fator: 1 },
        { uuid: "semaphore2", description: "Semaphore-2", carCount: 5, colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, emergency: false, fator: 2 }
      );
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining("Semaphore-1"));
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining("Semaphore-2"));
    });
  });
  ...
  describe("initializeSemaphores", () => {
    it("should increment cars in the semaphores every 5 seconds", () => {
      ...
      semaphores.push(
        { uuid: "semaphore1", carCount: 0, colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, emergency: false, fator: 1, description: "" },
        { uuid: "semaphore2", carCount: 0, colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, emergency: false, fator: 1, description: "" }
      );
  ...
  describe("dashboard", () => {
    it("should log the times and status of the semaphores", () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining("Verde"));
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining("Amarelo"));
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining("Vermelho"));
  ...
  describe("toggleSemaphores", () => {
    it("should toggle the semaphores based on the given parameters", () => {
      ...
      (toggleSemaphores as jest.Mock).mockImplementation((semaphoreIndex, stateIndex, rushActive) => {
        ...
        setTimeout(() => {
          toggleSemaphores((semaphoreIndex + 1) % semaphores.length, 0, rushActive);
        }, duration);
      });
  ...
  describe("checkEmergencyStatus", () => {
    it("should return true if the car count exceeds the emergency trigger", () => {
      semaphores.push(
        { uuid: "semaphore1", carCount: 25, colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, emergency: false, description: "", fator: 1 },
        { uuid: "semaphore2", carCount: 35, colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, emergency: false, description: "", fator: 1 }
      );
    ...
    it("should return false if the car count does not exceed the emergency trigger", () => {
      semaphores.push(
        { uuid: "semaphore1", carCount: 15, colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, emergency: false, description: "", fator: 1 },
        { uuid: "semaphore2", carCount: 20, colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, emergency: false, description: "", fator: 1 }
      );
  ...
  describe("handleGreen", () => {
    it("should reduce the car count when the green time is reached", () => {
      const semaphore: Semaphore = { uuid: "semaphore1", carCount: 10, colorStatus: ColorStatus.GREEN, location: { lat: 0, lon: 0 }, emergency: false, description: "", fator: 1 };
      timeAux = new Date();
  ...
  describe("handleYellow", () => {
    it("should reduce the car count when the yellow time is reached", () => {
      const semaphore: Semaphore = { uuid: "semaphore1", carCount: 5, colorStatus: ColorStatus.YELLOW, location: { lat: 0, lon: 0 }, emergency: false, description: "", fator: 1 };
      timeAux = new Date();
  ...
  describe("setSemaphoreEmergency", () => {
    it("should set the emergency status of the semaphore", () => {
      semaphores.push(
        { uuid: "semaphore1", emergency: false, colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, carCount: 0, description: "", fator: 1 },
        { uuid: "semaphore2", emergency: false, colorStatus: ColorStatus.RED, location: { lat: 0, lon: 0 }, carCount: 0, description: "", fator: 1 }
      );

      (setSemaphoreEmergency as jest.Mock).mockImplementation((uuid: string, emergency: boolean) => {
